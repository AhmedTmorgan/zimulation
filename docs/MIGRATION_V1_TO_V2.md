...
| 4 — learning agent | **done** — perception, memory, belief, causal inference done and verified; concepts (Anderson's rational model) and sensorimotor primitives done and verified; learned skills (reliable state changes chunked from the agent's own acts; stone-breaking found by babbling with real physics) done and verified; bounded satisficing planning over the agent's own experience, every decision's alternatives on record, done and verified. Section 18's counterfactual replay awaits the engine Drives with learned remedies (section 13) and a place memory added (cognition/goals.py, places.py). A living agent runs in an engine (behavior/arbitration.py, core/engine.py); measured honestly: over five days, using what was learned did not improve a lone agent survival, an innate mouthing prior (declared, off by default) did. |
